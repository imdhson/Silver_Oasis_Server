...
type Dj_users_users_settings struct {
	Loc      string `bson:"loc"`
	Service1 string `bson:"service1"`
	Service2 string `bson:"service2"`
	Service3 string `bson:"service3"`
}
...
func (a SO_jobs_detail_s) Swap(i int, j int) {
	a[i], a[j] = a[j], a[i]
}
func (a SO_jobs_detail_s) Less(i int, j int) bool {
